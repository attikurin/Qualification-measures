// gcp-cdl-progress.js - GCP Cloud Digital Leader講座の学習進捗管理・ブックマーク機能
...
const sections = [
    ...
    { id: 'services', name: '重要サービス', icon: 'fa-server' },
...
function updateQuizProgress() {
    ...
    const savedAnswers = localStorage.getItem('gcp_cdl_quiz_answers');
    if (savedAnswers) {
...
function saveProgress() {
    ...
    localStorage.setItem('gcp_cdl_progress', JSON.stringify(progressData));
}

function loadProgress() {
    const saved = localStorage.getItem('gcp_cdl_progress');
    if (saved) {
...
function saveBookmarks() {
    ...
    localStorage.setItem('gcp_cdl_bookmarks', JSON.stringify(bookmarksArray));
}

function loadBookmarks() {
    const saved = localStorage.getItem('gcp_cdl_bookmarks');
    if (saved) {
