...
function updateProgressDisplay() {
    ...
    if (typeof quizStats !== 'undefined') {
        ...
        if (rateEl) {
            const rate = quizStats.answered > 0 
...
function saveProgress() {
    if (typeof(Storage) !== "undefined") {
        ...
        localStorage.setItem('ipass_progress', JSON.stringify(progressData));
    }
}

function loadProgress() {
    if (typeof(Storage) !== "undefined") {
        const saved = localStorage.getItem('ipass_progress');
        if (saved) {
            try {
                const progressData = JSON.parse(saved);
                viewedSections = new Set(progressData.viewedSections || []);
...
function saveBookmarks() {
    if (typeof(Storage) !== "undefined") {
        localStorage.setItem('ipass_bookmarks', JSON.stringify(Array.from(bookmarks)));
    }
}

function loadBookmarks() {
    if (typeof(Storage) !== "undefined") {
        const saved = localStorage.getItem('ipass_bookmarks');
        if (saved) {
            try {
...
// 練習問題の解答が更新されたときに呼び出される
if (typeof window !== 'undefined') {
    window.updateProgressFromQuiz = function() {
        updateProgressDisplay();
    };
}
